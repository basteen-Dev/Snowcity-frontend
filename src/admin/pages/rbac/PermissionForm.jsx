...
export default function PermissionForm() {
  ...
  const [state, setState] = React.useState({
    ...
    error: null,
    form: { permission_key: '', description: '' }
  });
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const res = await adminApi.get(`${A.permissions()}/${id}`);
        const p = res?.permission || res || {};
        setState((s) => ({ ...s, status: 'idle', form: { permission_key: p.permission_key || '', description: p.description || '' } }));
      } catch (err) {
  ...
  return (
    <form onSubmit={save} className="max-w-xl bg-white border rounded-xl p-4">
      <h1 className="text-xl font-semibold mb-4">{isEdit ? 'Edit' : 'New'} Permission</h1>
      <label className="block text-sm text-gray-600 mb-1">Permission Key</label>
      <input className="w-full rounded-md border px-3 py-2 mb-3" value={f.permission_key} onChange={(e) => setState((s) => ({ ...s, form: { ...s.form, permission_key: e.target.value } }))} />
